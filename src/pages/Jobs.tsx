...
import { AGENT, JOBS, JOB_PRIVATE, openJobs, usd, type Job, type View } from '../data'
...
function Detail({ job, openJob }: { job: Job; openJob: (id: string | null) => void }) {
    ...
    <div>
      ...
      <p className="agendadate">
        ...
        {job.status === 'delivered'
          ...
          : job.status === 'running'
            ? `running · ~${job.etaMinutes} min left`
            : `queued · ~${job.etaMinutes} min`}
      </p>
      ...
      <div className="jobgrid">
        ...
        <div className="jf">
          <div className="k">{job.status === 'delivered' ? 'Delivered' : 'Estimate now'}</div>
          <div className="v">
            {job.status === 'delivered' ? job.deliveredAt : `~${job.etaMinutes} min`}
          </div>
        </div>
        ...
        <div className="jf">
          ...
          <div className="v">
            {job.receiptSig ? <a href="#">{job.receiptSig}</a> : <span className="mut">on delivery</span>}
          </div>
        </div>
        <div className="jf">
        ...
        <div className="jf wide locked">
          <div className="k">
            {unlocked ? priv.inputLabel : 'What was submitted'}
            <span className="lockbadge">payer only</span>
          </div>
          <div className={`v${unlocked ? '' : ' redacted'}`}>
            {unlocked ? priv.input : '••••••••••••••••••••••••••••••••••••••••'}
          </div>
        </div>
        <div className="jf wide locked">
...
export default function Jobs({ jobId, openJob }: Props) {
  ...
  if (job) return <Detail job={job} openJob={openJob} />
  ...
  return (
    <div>
      ...
      <p className="agendadate">
        {open.length} in the queue · day {AGENT.day} · public record
      </p>
